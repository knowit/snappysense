...
// type definitions:
//    struct tone_t { uint16_t frequency; uint16_t duration_ms; };
//    struct music_t { unsigned length; const struct tone_t* tones; };
...
func main() {
	...
	lim := len(tune)
	{
		...
		fmt.Printf("const struct tone_t %s[] = {", notes_name)
		for _, t := range music {
			fmt.Printf("{%d,%d},", t.frequency, t.duration_ms)
		}
		fmt.Printf("};\n")
		fmt.Printf("const struct music_t melody = { %d, %s };\n", len(music), notes_name)
		return
	}
